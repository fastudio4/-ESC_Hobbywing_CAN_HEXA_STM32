...
uint32_t _uavcan_protocol_SoftwareVersion_encode(struct uavcan_protocol_SoftwareVersion* msg, uint8_t* buffer
    ...
    __uavcan_protocol_SoftwareVersion_encode(buffer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
 ...
 */
bool _uavcan_protocol_SoftwareVersion_decode(const CanardRxTransfer* transfer, struct uavcan_protocol_SoftwareVersion* msg) {
#if CANARD_ENABLE_TAO_OPTION
...
#endif
    uint32_t bit_ofs = 0;
    if (__uavcan_protocol_SoftwareVersion_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
